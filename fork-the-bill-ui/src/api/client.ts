...
export const unclaimItem = async (slug: string, itemId: string, personName: string): Promise<Expense> => {
  try {
    // First, get the current expense to find person ID
    ...
    const response: AxiosResponse<ExpenseResponse> = await apiClient.delete(
      getApiUrl(API_CONFIG.ENDPOINTS.UNCLAIM_ITEM(slug, itemId, person.id))
    );
    return convertExpenseResponseToExpense(response.data);
  } catch (error) {
